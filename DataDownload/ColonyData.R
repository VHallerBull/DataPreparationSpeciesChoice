...
data <- read.csv("https://coraltraits.org/traits/155.csv", as.is=TRUE)
data <- data[data$trait_id==155,]
...
SpeciesData$colonyareaMin <- TraitData$Min
SpeciesData$colonyareaMax <- TraitData$Max
...
#coloniality
data <- read.csv("https://coraltraits.org/traits/104.csv", as.is=TRUE)
data <- data[data$trait_id==104,]
...
SpeciesData$coloniality <- TraitData$Min


#write.table(SpeciesData,"ReproductiveBeforeImpute.csv")
saveRDS(SpeciesData, file = "ColonyBeforeImpute.RData")
