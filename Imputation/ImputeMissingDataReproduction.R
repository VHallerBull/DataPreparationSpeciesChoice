...
SpeciesData<-readRDS(file = "~/SpeciesChoice/DataPreparationSpeciesChoice/DataDownload/ReproductiveBeforeImpute.RData")
...
png(file="Reproduction_Patternplot.png",width=1000)
...
for (i in 1:10){
  ...
  FileName <- paste('Reproduction_ImputedData',i,'.RData',sep="")
...
for (i in 1:10){
  ...
  for (n in 1:numbersets){
    ...
    png(file=paste(DataName,"c_Pattern.png"))
  ...
  for (p in 1:length(VariablesImputed)) {
    png(file=paste(FileName,"_",VariablesImputed[p],"c_plot.jpg",sep=""),width=1000)
