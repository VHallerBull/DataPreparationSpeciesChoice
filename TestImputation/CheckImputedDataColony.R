...
SpeciesData<-readRDS(file = "~/SpeciesChoice/DataPreparationSpeciesChoice/Imputation/ColonyBeforeImpute.RData")
...
SpeciesDataImp$colonydiameterMin[SpeciesDataImp$colonydiameterMin<0]<-NA
SpeciesDataImp$colonydiameterMin[SpeciesDataImp$colonydiameterMin=='NaN']<-NA
SpeciesDataImp$colonydiameterMax[SpeciesDataImp$colonydiameterMax=='NaN']<-NA
...
NotMissing <- filter(SpeciesDataImp,SpeciesDataImp$colonydiameterMin!='NA')
Loose <- NotMissing$ID
...
for (i in 3:3){
  ...
  SpeciesDataImp2$colonydiameterMin[NotMissing$ID[tries]]<- NA
  SpeciesDataImp2$colonydiameterMax[NotMissing$ID[tries]]<- NA
  ...
  for (r in 1: length(completeData)){
    completeData_reduced[[r]] <-subset(completeData[[r]],select=colonydiameterMin)
  }
  ...
  for (r in 1: length(completeData)){
    completeData_reduced[[r]] <-subset(completeData[[r]],select=colonydiameterMax)
  }
  ...
  #Create table
  FinalTable <- data.frame(OriginalMin=NotMissing$colonydiameterMin,
                          ImputedMinMin=MinMinimum,
                          ImputedMinMean=MinMean,
                          ImputedMinMax=MinMaximum,
                          OriginalMax=NotMissing$colonydiameterMin,
                          ImputedMaxMin=MaxMinimum,
                          ImputedMaxMean=MaxMean,
                          ImputedMaxMax=MaxMaximum)
  write.csv(FinalTable,file=paste('Colony_ImputedChecks',as.character(i),as.character(sets),'.csv',sep=""))
  write.csv(NotMissing,file='Colony_NotMissing.csv')
...
ResultsTable<-read.csv('Colony_ImputedChecks.csv')
