...
SpeciesData<-readRDS(file = "~/SpeciesChoice/DataPreparationSpeciesChoice/Imputation/PhotosynthesisBeforeImpute.RData")
...
SpeciesDataImp$photosynthesisMin[SpeciesDataImp$photosynthesisMin<0]<-NA
SpeciesDataImp$photosynthesisMin[SpeciesDataImp$photosynthesisMin=='NaN']<-NA
SpeciesDataImp$photosynthesisMax[SpeciesDataImp$photosynthesisMax=='NaN']<-NA
...
NotMissing <- filter(SpeciesDataImp,SpeciesDataImp$photosynthesisMin!='NA')
Loose <- NotMissing$ID
...
for (i in 2:3){
  ...
  SpeciesDataImp2$photosynthesisMin[NotMissing$ID[tries]]<- NA
  SpeciesDataImp2$photosynthesisMax[NotMissing$ID[tries]]<- NA
  ...
  for (r in 1: length(completeData)){
    completeData_reduced[[r]] <-subset(completeData[[r]],select=photosynthesisMin)
  }
  ...
  for (r in 1: length(completeData)){
    completeData_reduced[[r]] <-subset(completeData[[r]],select=photosynthesisMax)
  }
  ...
  #Create table
  FinalTable <- data.frame(OriginalMin=NotMissing$photosynthesisMin,
                          ImputedMinMin=MinMinimum,
                          ImputedMinMean=MinMean,
                          ImputedMinMax=MinMaximum,
                          OriginalMax=NotMissing$photosynthesisMin,
                          ImputedMaxMin=MaxMinimum,
                          ImputedMaxMean=MaxMean,
                          ImputedMaxMax=MaxMaximum)
  write.csv(FinalTable,file=paste('Photosynthesis_ImputedChecks',as.character(i),as.character(sets),'.csv',sep=""))
  write.csv(NotMissing,file='Photosynthesis_NotMissing.csv')
...
ResultsTable<-read.csv('Photosynthesis_ImputedChecks31.csv')
